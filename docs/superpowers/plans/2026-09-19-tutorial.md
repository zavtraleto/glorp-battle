...
`tests/terminalMenu.test.ts` builds its fixture with `session(screen, over)` (line 10). Add `tutorial: false` to that fixture's defaults so every existing test keeps compiling.

The existing assertion in `'builds each screen with its actions'` (line 36) lists the title items and **will fail** once the item is added — update it in the same edit:
